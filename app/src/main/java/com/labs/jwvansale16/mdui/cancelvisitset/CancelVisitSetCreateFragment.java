...
import com.labs.jwvansale16.databinding.FragmentCancelvisitsetCreateBinding;
...
/**
 * A fragment that presents a screen to either create or update an existing CancelVisit entity.
 * This fragment is contained in the {@link CancelVisitSetActivity}.
 */
public class CancelVisitSetCreateFragment extends InterfacedFragment<CancelVisit> {

    private static final Logger LOGGER = LoggerFactory.getLogger(CancelVisitSetCreateFragment.class);
    ...
    /** CancelVisit object and it's copy: the modifications are done on the copied object. */
    private CancelVisit cancelVisitEntity;
    private CancelVisit cancelVisitEntityCopy;

    /** DataBinding generated class */
    private FragmentCancelvisitsetCreateBinding binding;

    /** Indicate what operation to be performed */
    private String operation;

    /** CancelVisit ViewModel */
    ...
    /**
     * This fragment is used for both update and create for CancelVisitSet to enter values for the properties.
     ...
     *            CancelVisit if Operation is update
     */
    @Override
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (UIConstants.OP_CREATE.equals(operation)) {
            activityTitle = currentActivity.getResources().getString(R.string.title_create_fragment, EntityTypes.cancelVisit.getLocalName());
        } else {
            activityTitle = currentActivity.getResources().getString(R.string.title_update_fragment) + " " + EntityTypes.cancelVisit.getLocalName();
        }

        ((CancelVisitSetActivity)currentActivity).isNavigationDisabled = true;
        ...
        if(UIConstants.OP_CREATE.equals(operation)) {
            cancelVisitEntity = createCancelVisit();
        } else {
            cancelVisitEntity = viewModel.getSelectedEntity().getValue();
        }

        CancelVisit workingCopy = null;
        if( savedInstanceState != null ) {
            workingCopy =  (CancelVisit)savedInstanceState.getParcelable(KEY_WORKING_COPY);
        }
        if( workingCopy == null ) {
            cancelVisitEntityCopy = (CancelVisit) cancelVisitEntity.copy();
            cancelVisitEntityCopy.setEntityTag(cancelVisitEntity.getEntityTag());
            cancelVisitEntityCopy.setOldEntity(cancelVisitEntity);
            cancelVisitEntityCopy.setEditLink((cancelVisitEntity.getEditLink()));
        } else {
            //in this case, the old entity and entity tag should already been set.
            cancelVisitEntityCopy = workingCopy;
        }
    }
    ...
    public void onSaveInstanceState(@NonNull Bundle outState) {
        outState.putParcelable(KEY_WORKING_COPY, cancelVisitEntityCopy);
    ...
    private boolean onSaveItem() {
        if (!isCancelVisitValid()) {
            return false;
        }
        //set 'isNavigationDisabled' false here to make sure the logic in list is ok, and set it to true if update fails.
        ((CancelVisitSetActivity)currentActivity).isNavigationDisabled = false;
        if( progressBar != null ) progressBar.setVisibility(View.VISIBLE);
        if (operation.equals(UIConstants.OP_CREATE)) {
            viewModel.create(cancelVisitEntityCopy);
        } else {
            viewModel.update(cancelVisitEntityCopy);
        }
        return true;
    }

    /**
     * Create a new CancelVisit instance and initialize properties to its default values
     * Nullable property will remain null
     * For offline, keys will be unset to avoid collision should more than one is created locally
     * @return new CancelVisit instance
     */
    private CancelVisit createCancelVisit() {
        CancelVisit cancelVisitEntity = new CancelVisit(true);
        cancelVisitEntity.unsetDataValue(CancelVisit.driverid);
        cancelVisitEntity.unsetDataValue(CancelVisit.visitid);
        cancelVisitEntity.unsetDataValue(CancelVisit.visititemno);
        cancelVisitEntity.unsetDataValue(CancelVisit.customerno);
        return cancelVisitEntity;
    }

    /** Callback function to complete processing when updateResult or createResult events fired */
    private void onComplete(@NonNull OperationResult<CancelVisit> result) {
        ...
        if (result.getError() != null) {
            ...
            if( UIConstants.OP_UPDATE.equals(operation) && !isMasterDetail) {
                viewModel.setSelectedEntity(cancelVisitEntityCopy);
            }
            if( isMasterDetail ) {
                CancelVisitSetListFragment listFragment = (CancelVisitSetListFragment) currentActivity.getSupportFragmentManager().findFragmentByTag(UIConstants.LIST_FRAGMENT_TAG);
    ...
    private View setupDataBinding(@NonNull LayoutInflater inflater, ViewGroup container) {
        binding = FragmentCancelvisitsetCreateBinding.inflate(inflater, container, false);
        View rootView = binding.getRoot();
        binding.setCancelVisit(cancelVisitEntityCopy);
        return rootView;
    }

    /** Validate the edited inputs */
    private boolean isCancelVisitValid() {
        LinearLayout linearLayout = getView().findViewById(R.id.create_update_cancelvisit);
        boolean isValid = true;
        ...
        for (int i = 0; i < linearLayout.getChildCount(); i++) {
            ...
            Property property = EntityTypes.cancelVisit.getProperty(propertyName);
     ...
     */
    private void handleError(@NonNull OperationResult<CancelVisit> result) {
        String errorMessage;
