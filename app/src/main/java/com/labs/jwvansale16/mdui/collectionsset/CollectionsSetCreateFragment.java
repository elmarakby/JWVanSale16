package com.labs.jwvansale16.mdui.collectionsset;
...
import com.labs.jwvansale16.databinding.FragmentCollectionssetCreateBinding;
...
/**
 * A fragment that presents a screen to either create or update an existing Collections entity.
 * This fragment is contained in the {@link CollectionsSetActivity}.
 */
public class CollectionsSetCreateFragment extends InterfacedFragment<Collections> {

    private static final Logger LOGGER = LoggerFactory.getLogger(CollectionsSetCreateFragment.class);
    ...
    /** Collections object and it's copy: the modifications are done on the copied object. */
    private Collections collectionsEntity;
    private Collections collectionsEntityCopy;

    /** DataBinding generated class */
    private FragmentCollectionssetCreateBinding binding;
    ...
    /**
     * This fragment is used for both update and create for CollectionsSet to enter values for the properties.
     ...
     *            Collections if Operation is update
     */
    @Override
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (UIConstants.OP_CREATE.equals(operation)) {
            activityTitle = currentActivity.getResources().getString(R.string.title_create_fragment, EntityTypes.collections.getLocalName());
        } else {
            activityTitle = currentActivity.getResources().getString(R.string.title_update_fragment) + " " + EntityTypes.collections.getLocalName();
        }

        ((CollectionsSetActivity)currentActivity).isNavigationDisabled = true;
        viewModel = new ViewModelProvider(currentActivity).get(CollectionsViewModel.class);
        ...
        if(UIConstants.OP_CREATE.equals(operation)) {
            collectionsEntity = createCollections();
        } else {
            collectionsEntity = viewModel.getSelectedEntity().getValue();
        }

        Collections workingCopy = null;
        if( savedInstanceState != null ) {
            workingCopy =  (Collections)savedInstanceState.getParcelable(KEY_WORKING_COPY);
        }
        if( workingCopy == null ) {
            collectionsEntityCopy = (Collections) collectionsEntity.copy();
            collectionsEntityCopy.setEntityTag(collectionsEntity.getEntityTag());
            collectionsEntityCopy.setOldEntity(collectionsEntity);
            collectionsEntityCopy.setEditLink((collectionsEntity.getEditLink()));
        } else {
            //in this case, the old entity and entity tag should already been set.
            collectionsEntityCopy = workingCopy;
        }
    }
    ...
    public void onSaveInstanceState(@NonNull Bundle outState) {
        outState.putParcelable(KEY_WORKING_COPY, collectionsEntityCopy);
    ...
    private boolean onSaveItem() {
        if (!isCollectionsValid()) {
            return false;
        }
        //set 'isNavigationDisabled' false here to make sure the logic in list is ok, and set it to true if update fails.
        ((CollectionsSetActivity)currentActivity).isNavigationDisabled = false;
        if( progressBar != null ) progressBar.setVisibility(View.VISIBLE);
        if (operation.equals(UIConstants.OP_CREATE)) {
            viewModel.create(collectionsEntityCopy);
        } else {
            viewModel.update(collectionsEntityCopy);
        }
        return true;
    }

    /**
     * Create a new Collections instance and initialize properties to its default values
     * Nullable property will remain null
     * For offline, keys will be unset to avoid collision should more than one is created locally
     * @return new Collections instance
     */
    private Collections createCollections() {
        Collections collectionsEntity = new Collections(true);
        collectionsEntity.unsetDataValue(Collections.userid);
        collectionsEntity.unsetDataValue(Collections.driverid);
        collectionsEntity.unsetDataValue(Collections.customerno);
        collectionsEntity.unsetDataValue(Collections.billno);
        collectionsEntity.unsetDataValue(Collections.externaldocument);
        return collectionsEntity;
    }

    /** Callback function to complete processing when updateResult or createResult events fired */
    private void onComplete(@NonNull OperationResult<Collections> result) {
        ...
        if (result.getError() != null) {
            ...
            if( UIConstants.OP_UPDATE.equals(operation) && !isMasterDetail) {
                viewModel.setSelectedEntity(collectionsEntityCopy);
            }
            if( isMasterDetail ) {
                CollectionsSetListFragment listFragment = (CollectionsSetListFragment) currentActivity.getSupportFragmentManager().findFragmentByTag(UIConstants.LIST_FRAGMENT_TAG);
    ...
    private View setupDataBinding(@NonNull LayoutInflater inflater, ViewGroup container) {
        binding = FragmentCollectionssetCreateBinding.inflate(inflater, container, false);
        View rootView = binding.getRoot();
        binding.setCollections(collectionsEntityCopy);
        return rootView;
    }

    /** Validate the edited inputs */
    private boolean isCollectionsValid() {
        LinearLayout linearLayout = getView().findViewById(R.id.create_update_collections);
        boolean isValid = true;
        ...
        for (int i = 0; i < linearLayout.getChildCount(); i++) {
            ...
            Property property = EntityTypes.collections.getProperty(propertyName);
     ...
     */
    private void handleError(@NonNull OperationResult<Collections> result) {
        String errorMessage;
