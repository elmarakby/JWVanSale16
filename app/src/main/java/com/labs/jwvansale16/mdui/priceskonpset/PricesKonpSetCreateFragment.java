...
import com.labs.jwvansale16.databinding.FragmentPriceskonpsetCreateBinding;
...
/**
 * A fragment that presents a screen to either create or update an existing PricesKonp entity.
 * This fragment is contained in the {@link PricesKonpSetActivity}.
 */
public class PricesKonpSetCreateFragment extends InterfacedFragment<PricesKonp> {

    private static final Logger LOGGER = LoggerFactory.getLogger(PricesKonpSetCreateFragment.class);
    ...
    /** PricesKonp object and it's copy: the modifications are done on the copied object. */
    private PricesKonp pricesKonpEntity;
    private PricesKonp pricesKonpEntityCopy;

    /** DataBinding generated class */
    private FragmentPriceskonpsetCreateBinding binding;

    /** Indicate what operation to be performed */
    private String operation;

    /** PricesKonp ViewModel */
    ...
    /**
     * This fragment is used for both update and create for PricesKonpSet to enter values for the properties.
     ...
     *            PricesKonp if Operation is update
     */
    @Override
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (UIConstants.OP_CREATE.equals(operation)) {
            activityTitle = currentActivity.getResources().getString(R.string.title_create_fragment, EntityTypes.pricesKonp.getLocalName());
        } else {
            activityTitle = currentActivity.getResources().getString(R.string.title_update_fragment) + " " + EntityTypes.pricesKonp.getLocalName();
        }

        ((PricesKonpSetActivity)currentActivity).isNavigationDisabled = true;
        ...
        if(UIConstants.OP_CREATE.equals(operation)) {
            pricesKonpEntity = createPricesKonp();
        } else {
            pricesKonpEntity = viewModel.getSelectedEntity().getValue();
        }

        PricesKonp workingCopy = null;
        if( savedInstanceState != null ) {
            workingCopy =  (PricesKonp)savedInstanceState.getParcelable(KEY_WORKING_COPY);
        }
        if( workingCopy == null ) {
            pricesKonpEntityCopy = (PricesKonp) pricesKonpEntity.copy();
            pricesKonpEntityCopy.setEntityTag(pricesKonpEntity.getEntityTag());
            pricesKonpEntityCopy.setOldEntity(pricesKonpEntity);
            pricesKonpEntityCopy.setEditLink((pricesKonpEntity.getEditLink()));
        } else {
            //in this case, the old entity and entity tag should already been set.
            pricesKonpEntityCopy = workingCopy;
        }
    }
    ...
    public void onSaveInstanceState(@NonNull Bundle outState) {
        outState.putParcelable(KEY_WORKING_COPY, pricesKonpEntityCopy);
    ...
    private boolean onSaveItem() {
        if (!isPricesKonpValid()) {
            return false;
        }
        //set 'isNavigationDisabled' false here to make sure the logic in list is ok, and set it to true if update fails.
        ((PricesKonpSetActivity)currentActivity).isNavigationDisabled = false;
        if( progressBar != null ) progressBar.setVisibility(View.VISIBLE);
        if (operation.equals(UIConstants.OP_CREATE)) {
            viewModel.create(pricesKonpEntityCopy);
        } else {
            viewModel.update(pricesKonpEntityCopy);
        }
        return true;
    }

    /**
     * Create a new PricesKonp instance and initialize properties to its default values
     * Nullable property will remain null
     * For offline, keys will be unset to avoid collision should more than one is created locally
     * @return new PricesKonp instance
     */
    private PricesKonp createPricesKonp() {
        PricesKonp pricesKonpEntity = new PricesKonp(true);
        pricesKonpEntity.unsetDataValue(PricesKonp.knumh);
        return pricesKonpEntity;
    }

    /** Callback function to complete processing when updateResult or createResult events fired */
    private void onComplete(@NonNull OperationResult<PricesKonp> result) {
        ...
        if (result.getError() != null) {
            ...
            if( UIConstants.OP_UPDATE.equals(operation) && !isMasterDetail) {
                viewModel.setSelectedEntity(pricesKonpEntityCopy);
            }
            if( isMasterDetail ) {
                PricesKonpSetListFragment listFragment = (PricesKonpSetListFragment) currentActivity.getSupportFragmentManager().findFragmentByTag(UIConstants.LIST_FRAGMENT_TAG);
    ...
    private View setupDataBinding(@NonNull LayoutInflater inflater, ViewGroup container) {
        binding = FragmentPriceskonpsetCreateBinding.inflate(inflater, container, false);
        View rootView = binding.getRoot();
        binding.setPricesKonp(pricesKonpEntityCopy);
        return rootView;
    }

    /** Validate the edited inputs */
    private boolean isPricesKonpValid() {
        LinearLayout linearLayout = getView().findViewById(R.id.create_update_priceskonp);
        boolean isValid = true;
        ...
        for (int i = 0; i < linearLayout.getChildCount(); i++) {
            ...
            Property property = EntityTypes.pricesKonp.getProperty(propertyName);
     ...
     */
    private void handleError(@NonNull OperationResult<PricesKonp> result) {
        String errorMessage;
